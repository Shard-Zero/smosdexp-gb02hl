...
const OrderlyProvider = (props: { children: ReactNode }) => {
	...
	// ENDPOINT for Live Preview: Listen for config updates and re-render
	useEffect(() => {
		const handleConfigUpdate = () => {
			console.log("OrderlyProvider: Config updated, re-rendering");
			setConfigVersion(v => v + 1);
		};
		window.addEventListener("runtime-config-updated", handleConfigUpdate);
		return () => window.removeEventListener("runtime-config-updated", handleConfigUpdate);
	}, []);
